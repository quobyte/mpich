...
char *ADIOI_QUOBYTEFS_ExtractRegistry(const char *filename, int *error_code)
{
    // input: //registry.address/[volume/]path
    // output: registry.address
    static char myname[] = "ADIOI_QUOBYTEFS_ExtractRegistry";
    const char *prefix = "//";
...
void ADIOI_QUOBYTEFS_CreateAdapter(const char *filename, int *error_code)
{
    ...
    char *registry = ADIOI_QUOBYTEFS_ExtractRegistry(filename, error_code);
...
char *GetVolumeAndPath(const char *filename)
{
    char *path = "";
